...
gait_files_src_path = os.path.join(
    get_package_share_directory(gait_files_package),
    "..",
    "..",
    "..",
    "..",
...
class TestJointGaitGenerator(Node):
    def __init__(self):
        ...
        self.gait_directory = (
            self.get_parameter("gait_directory").get_parameter_value().string_value
        )
        self.gait_name = (
            self.get_parameter("gait_name").get_parameter_value().string_value
        )
        self.subgait_name = (
            self.get_parameter("subgait_name").get_parameter_value().string_value
        )
        ...
        self.num_setpoints = (
            self.get_parameter("num_setpoints").get_parameter_value().integer_value
        )
        ...
        self.joint = JointTrajectory.get_joint_from_urdf(
            robot=self.robot, joint_name=self.joint_name
        )
    ...
    def generate_and_write_all(self):
        ...
        self.write_to_all_directories(
            gait, os.path.join(self.gait_name, f"{self.gait_name}.gait")
        )
        self.write_to_all_directories(
            subgait,
    ...
    def make_subgait_directory(self, gait_files_path):
        ...
        directory = os.path.join(
            gait_files_path, self.gait_directory, self.gait_name, self.subgait_name
        )
    ...
    def generate_default_yaml(self):
        ...
        default = {
            "gaits": {self.gait_name: {self.subgait_name: self.version}},
            "positions": {
                "setup": {
                    "gait_type": "",
                    "joints": {self.joint_name: self.start_position},
                }
            },
        }
    ...
    def generate_gait(self):
        ...
        gait = {
            "name": self.gait_name,
            "subgaits": {
                "start": {"to": self.subgait_name},
                self.subgait_name: {"to": "end"},
            },
        }
    ...
    def generate_setpoints(self):
        ...
        positions = [
            self.limits.lower if i % 2 == 0 else self.limits.upper
    ...
    def generate_description(self):
        ...
        return (
            f"Subgait generated using TestJointGaitGenerator, with a "
            f"duration of {self.duration}s and {self.num_setpoints} setpoints."
        )

    @staticmethod
    def make_setpoint_dict(
        seconds: int, nanoseconds: int, position: float, velocity: float
    ) -> dict:
        """ Create a setpoint dictionary."""
        return {
            "time_from_start": {"secs": seconds, "nsecs": nanoseconds},
            "position": position,
            "velocity": velocity,
        }
