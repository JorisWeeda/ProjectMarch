...
from rclpy.callback_groups import ReentrantCallbackGroup, MutuallyExclusiveCallbackGroup, CallbackGroup
...
class DynamicPIDReconfigurer(Node):
    ...
    def gait_selection_callback(self, msg):
        ...
        self.get_logger().info(str(self.callback_group))
        ...
        self.callback_num += 1
        curr_callback_num = self.callback_num

        if not self.is_interpolation_done(needed_gains):
            self.start_interpolation(needed_gains, curr_callback_num)

    def start_interpolation(self, needed_gains: np.ndarray, curr_callback_num):
        rate = self.create_rate(10)

        self.get_logger().info(f'Beginning PID interpolation for gait type: {self._gait_type}')
        begin_time = self.get_clock().now()
        self._last_update_time = begin_time
        while not self.is_interpolation_done(needed_gains):
            self.client_update(needed_gains)

            if self.callback_num != curr_callback_num:
                self.get_logger().info('New callback, closing this thread')
                return

            rate.sleep()
        duration_ms = round((self.get_clock().now() - begin_time).nanoseconds / 1e6)
        self.get_logger().info(f'PID interpolation finished in {duration_ms} ms')

    def client_update(self, needed_gains: np.ndarray):
        """
        ...
        if self._linearize:
            ...
            time_interval = (current_time - self._last_update_time).nanoseconds / 1e9


            self.get_logger().info(f'Calling interpolation() with {time_interval}:')
            self.current_gains = interpolate(self.current_gains, needed_gains, self._gradient, time_interval)
            # self.get_logger().info('Done interpolation()')
    ...
    def get_gains_callback(self, future_done: Future):
        ...
        result = future_done.result()
        if result.success:
            self.current_gains = np.array([gain_values.gains for gain_values in result.gain_values.gain_values_list])
        else:
            raise ParameterGetException(self._joint_list)
        self.get_current_gains_event.set()

    def get_needed_gains(self) -> np.ndarray:
        """
    ...
    def is_interpolation_done(self, needed_gains: np.ndarray) -> bool:
        """
        ...
        """
        return (self.current_gains == needed_gains).all()


class OverwritingCallbackGroup(CallbackGroup):
